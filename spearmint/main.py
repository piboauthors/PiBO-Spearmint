...
def main():
    ...
    elif 'p0add' in sys.argv[3]:
        added_samples = int(sys.argv[3][5:])
        model = 'p0'
    prior_strength = sys.argv[4]
    run_nbr = str(sys.argv[5])
    try:
        output_name = sys.argv[6]
        options['database']['address'] = 'db/localhost' + sys.argv[6]
        ...
        options['database']['address'] = 'db/localhost' + exp + model + prior_strength + run_nbr
    ...
    res_path = current_dir + model + output_name + '.csv' 
    conf_path = current_dir + 'configs' + model  + output_name + '.csv'

    iteration = 0
    last_save_iteration = 0
    # should probably set a ceiling to this
    while iteration < MAX_ITERS + added_samples:
