...
def init(options, prior):
    return DefaultChooser(options, prior)
...
class DefaultChooser(object):
    ...
    objective : dict
        ?
    """
    def __init__(self, options, prior=None):
    ...
    def suggest(self, prior):
        if 'p0add' in sys.argv[3]:
            added_samples = int(sys.argv[3][5:])

        else:
            added_samples = 0
        if 'sample' in sys.argv[3]: 
            all_samples = True
        else: 
            all_samples = False
        ...
        if self.objective['inputs'].shape[0] < DEFAULT_NUMDESIGN + added_samples:
            if self.prior != None:
                if self.objective['inputs'].shape[0] == 0  and not all_samples:
                    suggestion = prior.get_max_location()
                    print('Initialized with mode of prior:', suggestion)
                
                elif (added_samples == 0) or (self.objective['inputs'].shape[0] < added_samples):
                    suggestion = self.prior.sample_from_prior().reshape(-1)
                    print('\nThis configuration was sampled from the prior.\n')
                
                else:
                    print('\nNo more freebies, initializing from SOBOL.\n')
                    suggestion = self.task_group.from_unit(self.grid[self.design_index-added_samples])
                
            else:
                print('\nInitializing from SOBOL grid.\n')
                suggestion = self.task_group.from_unit(self.grid[self.design_index-added_samples])
        ...
        # TODO - if prior, use the prior version - probably only need to change acq_function
        ...
        if self.parallel_opt:
            ...
            # Optimize in series
            for idx, c in enumerate(best_grid_pred):
        ...
        test_grid = sobol_grid.generate(self.num_dims, grid_size=self.grid_size, 
                                        grid_seed=self.grid_seed)
        ...
        # TODO update the time step on the prior
        if self.prior:
            self.prior.update()
    ...
    def acquisition_function(self, cand, current_best, compute_grad=True):
        ...
        if self.numConstraints() == 0:
            if self.prior == None:
                res = compute_ei(obj_model, cand, ei_target=current_best, compute_grad=compute_grad)
                return compute_ei(obj_model, cand, ei_target=current_best, compute_grad=compute_grad)

            else:
                return self.prior._compute(obj_model, cand, ei_target=current_best, compute_grad=compute_grad)
        if cand.ndim == 1:
            cand = cand[None]
    ...
    def get_all_values(self):
        return self.task_group.values['main']

    def evaluate_at_prior(self):
        pass

    def evaluate_at_optimum(self):
        pass     
