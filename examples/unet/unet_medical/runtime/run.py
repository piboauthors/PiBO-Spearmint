...
from unet_medical.runtime.losses import partial_losses
from unet_medical.runtime.parse_results import process_performance_stats


def train(params, model, dataset, logger):
    np.random.seed(params.seed)
    tf.random.set_seed(params.seed)
    max_steps = params.max_steps // 1#hvd.size()

    optimizer = tf.keras.optimizers.Adam(learning_rate=params.learning_rate, beta_1=params.beta_1, beta_2=params.beta_2)
    if params.use_amp:
    ...
    if params.benchmark:
        assert max_steps * 1 > params.warmup_steps, \
        ...
        for iteration, (images, labels) in enumerate(dataset.train_fn(drop_remainder=True)):
            ...
            if iteration >= max_steps * 1:#hvd.size():
                break

        if True:#hvd.rank() == 0:
            deltas = np.array([timestamps[i + 1] - timestamps[i] for i in range(len(timestamps) - 1)])
            stats = process_performance_stats(deltas, 1 * params.batch_size, mode="train")
...
def evaluate(params, model, dataset, logger, restore_checkpoint=True):
    ...
    logger.flush()
    return 1.0 - float(f1_loss.result())  # eval_dice_score
