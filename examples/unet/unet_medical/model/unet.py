...
from unet_medical.model.layers import InputBlock, DownsampleBlock, BottleneckBlock, UpsampleBlock, OutputBlock
...
class Unet(tf.keras.Model):
    ...
    def __init__(self, params):
        super().__init__(self)
        self.input_block = InputBlock(filters=64, params=params)
        self.bottleneck = BottleneckBlock(1024, params=params)
        self.output_block = OutputBlock(filters=64, n_classes=2, params=params)

        self.down_blocks = [DownsampleBlock(filters, idx, params)
                            for idx, filters in enumerate([128, 256, 512])]

        self.up_blocks = [UpsampleBlock(filters, idx, params)
