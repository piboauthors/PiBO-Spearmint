...
def eval_function(args):
    container_source = join(dirname(dirname(abspath(__file__))), 'libs/containers/tabular_benchmark')
    print(dirname(dirname(abspath(__file__))))
    print('CONTAINER_SOURCE', container_source)

    b = Benchmark(task_id=146818, rng=1, model='nn', container_source=container_source)
